...
function renderGrid(page=1){
  ...
  list.slice(start,start+per).forEach((item,i)=>{
    ...
    div.innerHTML='<img src="'+esc(item.imageUrl)+'" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" alt="포트폴리오"><div class="absolute inset-0 bg-slate-900/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"><span class="text-white font-bold border border-white px-3 py-1 text-xs uppercase tracking-widest">View</span></div>';
