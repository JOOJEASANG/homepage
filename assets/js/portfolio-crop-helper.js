...
function injectStyle() {
  ...
  style.textContent = `
    ...
    .pcrop-body{padding:18px;background:#f8fafc}.pcrop-stage{width:min(100%,420px);aspect-ratio:1/1;margin:0 auto;border-radius:18px;overflow:hidden;background:#111827;box-shadow:inset 0 0 0 1px rgba(255,255,255,.18);cursor:grab;touch-action:none;}.pcrop-stage:active{cursor:grabbing}.pcrop-stage canvas{width:100%;height:100%;display:block;}
    .pcrop-tools{margin:16px auto 0;width:min(100%,420px);display:grid;gap:10px}.pcrop-tools label{font-size:12px;font-weight:900;color:#475569}.pcrop-tools input[type=range]{width:100%;accent-color:#16a34a}.pcrop-actions{display:flex;gap:10px;justify-content:flex-end;padding:14px 18px;border-top:1px solid #e2e8f0;background:#fff}.pcrop-btn{border:0;border-radius:12px;padding:10px 14px;font-weight:900;font-size:13px;cursor:pointer}.pcrop-cancel{background:#f1f5f9;color:#475569}.pcrop-save{background:#16a34a;color:#fff}.pcrop-save:disabled{opacity:.5;cursor:not-allowed}
  `;
...
function modal() {
  ...
  m.innerHTML = `
    <div class="pcrop-box">
      <div class="pcrop-head"><div><b>포트폴리오 이미지 맞춤</b><p>마우스로 위치를 맞추고 확대/축소 후 1:1 정사각형으로 저장합니다.</p></div><button type="button" class="pcrop-close" data-pcrop-close>×</button></div>
      <div class="pcrop-body"><div class="pcrop-stage"><canvas id="portfolioCropCanvas" width="${EDITOR_SIZE}" height="${EDITOR_SIZE}"></canvas></div><div class="pcrop-tools"><label for="portfolioCropZoom">확대 / 축소</label><input id="portfolioCropZoom" type="range" min="1" max="4" step="0.01" value="1"></div></div>
...
async function openCrop(input, file) {
  ...
  image.onload = () => {
    ...
    const baseScale = Math.max(EDITOR_SIZE / image.width, EDITOR_SIZE / image.height);
